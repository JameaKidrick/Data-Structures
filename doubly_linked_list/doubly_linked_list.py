...
class DoublyLinkedList:
    ...
    def add_to_head(self, value):
        # create a node (ListNode(value))
        # this needs to be head since we are adding to head
        # update previous head
        # update length
        self.length += 1
        # edge cases?
        #   -- if self.head is none then there is no list and no tail
        #   -- if self.tail is none then new head also becomes new tail 
        if not self.head and not self.tail:
            # empty list, this is head and tail
            self.head = self.tail = ListNode(value)
        else:
            # list is populated, update head and tail
